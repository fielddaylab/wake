...
namespace Aqua {
    ...
    public class ShakeAnim : MonoBehaviour, ILayoutAnim
    {
        private const float AnimDistance = -4;
        private const float AnimDuration = 8 / 60f;
        private const float AnimCycles = 3;
        ...
        private void Apply(float percent) {
            float amt = Wave.CosFade(percent, AnimCycles);
            m_Offset.Offset0 = new Vector2(m_AnimDistanceScale * AnimDistance * amt, 0);
        }
