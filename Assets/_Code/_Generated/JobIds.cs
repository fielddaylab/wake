using System;
using BeauUtil;

static public class JobIds
{
	static public readonly StringHash32 Kelp_urchin_barren_predict = new StringHash32(0x1370E8F8);
	static public readonly StringHash32 Kelp_refuge_failure_simulation = new StringHash32(0x1BE97A2D);
	static public readonly StringHash32 Kelp_urchin_farm = new StringHash32(0x5759A6BA);
	static public readonly StringHash32 Kelp_save_urchin_barren = new StringHash32(0x59D83768);
	static public readonly StringHash32 Kelp_bull_kelp_forest = new StringHash32(0x5E9238A2);
	static public readonly StringHash32 Kelp_musselfest_solution = new StringHash32(0x5E9DBFDD);
	static public readonly StringHash32 Kelp_start_refuge = new StringHash32(0x6FB36AAF);
	static public readonly StringHash32 Kelp_shop_welcome = new StringHash32(0x8460E9DD);
	static public readonly StringHash32 Kelp_urchin_barren_viz = new StringHash32(0x89B20654);
	static public readonly StringHash32 Kelp_energy = new StringHash32(0x903BE990);
	static public readonly StringHash32 Kelp_welcome = new StringHash32(0x92603728);
	static public readonly StringHash32 Kelp_refuge_failure = new StringHash32(0xA9C5D9D3);
	static public readonly StringHash32 Kelp_mussel_fest = new StringHash32(0xE6B6CBE6);
	static public readonly StringHash32 Coral_turtle_stability = new StringHash32(0x0DCB739D);
	static public readonly StringHash32 Coral_fake_fix = new StringHash32(0x1751965E);
	static public readonly StringHash32 Coral_stressed = new StringHash32(0x2DB0F634);
	static public readonly StringHash32 Coral_fishy_bizz = new StringHash32(0x2FFC356C);
	static public readonly StringHash32 Coral_lionfish_conspiracy = new StringHash32(0x4CCF110B);
	static public readonly StringHash32 Coral_much_algae = new StringHash32(0x762958B9);
	static public readonly StringHash32 Coral_eat_seaweed = new StringHash32(0x8BB21CB6);
	static public readonly StringHash32 Coral_ocean_plastics = new StringHash32(0x95516B0D);
	static public readonly StringHash32 Coral_hunting_lions = new StringHash32(0x99E410F2);
	static public readonly StringHash32 Coral_casting_shade = new StringHash32(0xC3AFF6BA);
	static public readonly StringHash32 Coral_turtle_population = new StringHash32(0xEE7D2D39);
	static public readonly StringHash32 Coral_tang_checkup = new StringHash32(0xFEDB36CD);
	static public readonly StringHash32 Bayou_dirty_detritus = new StringHash32(0x1EE52225);
	static public readonly StringHash32 Bayou_reef_decision = new StringHash32(0x6E8EAD11);
	static public readonly StringHash32 Turtle_danger = new StringHash32(0x720AABC5);
	static public readonly StringHash32 Displaced_reef = new StringHash32(0x72DB0EAF);
	static public readonly StringHash32 Turtle_danger2 = new StringHash32(0x7DCCB5D5);
	static public readonly StringHash32 Bayou_blue_waters = new StringHash32(0x9E14FCD5);
	static public readonly StringHash32 Bayou_hide_n_seek = new StringHash32(0xA329BE68);
	static public readonly StringHash32 Bayou_oxygen_tracking = new StringHash32(0xADC978AC);
	static public readonly StringHash32 Bayou_shrimp_yields = new StringHash32(0xBEC671F2);
	static public readonly StringHash32 Bayou_methanogen = new StringHash32(0xD7BE7F32);
	static public readonly StringHash32 Bayou_shrimp_tastrophe = new StringHash32(0xDFEDE55E);
	static public readonly StringHash32 Bayou_alt_energy = new StringHash32(0xED0F1F74);
	static public readonly StringHash32 Bayou_boom_cause = new StringHash32(0xF3776BEB);
	static public readonly StringHash32 Arctic_disappearing_act = new StringHash32(0x0971100E);
	static public readonly StringHash32 Arctic_underneath = new StringHash32(0x10E1380E);
	static public readonly StringHash32 Arctic_endangered_seals = new StringHash32(0x4452693A);
	static public readonly StringHash32 Above_n_below = new StringHash32(0x490AFAC1);
	static public readonly StringHash32 Arctic_time_of_death = new StringHash32(0x50882F92);
	static public readonly StringHash32 Arctic_isolated_instance = new StringHash32(0x6A793671);
	static public readonly StringHash32 Arctic_picky_eaters = new StringHash32(0x75B529D9);
	static public readonly StringHash32 Arctic_salmon_monitoring = new StringHash32(0x81F0F243);
	static public readonly StringHash32 Arctic_missing_whale = new StringHash32(0x90AB11F0);
	static public readonly StringHash32 Arctic_in_ice = new StringHash32(0xA1466A9D);
	static public readonly StringHash32 Arctic_cause_of_death = new StringHash32(0xB437D958);
	static public readonly StringHash32 Arctic_salmon_competition = new StringHash32(0xB5BE46DC);
	static public readonly StringHash32 Arctic_stationary_survival = new StringHash32(0xB78FCBB7);
	static public readonly StringHash32 Arctic_seal_habitats = new StringHash32(0xECA830FC);
	static public readonly StringHash32 Final_final = new StringHash32(0xE6B6DB4A);

	static public readonly StringHash32[] All = new StringHash32[]
	{
		Kelp_urchin_barren_predict,
		Kelp_refuge_failure_simulation,
		Kelp_urchin_farm,
		Kelp_save_urchin_barren,
		Kelp_bull_kelp_forest,
		Kelp_musselfest_solution,
		Kelp_start_refuge,
		Kelp_shop_welcome,
		Kelp_urchin_barren_viz,
		Kelp_energy,
		Kelp_welcome,
		Kelp_refuge_failure,
		Kelp_mussel_fest,
		Coral_turtle_stability,
		Coral_fake_fix,
		Coral_stressed,
		Coral_fishy_bizz,
		Coral_lionfish_conspiracy,
		Coral_much_algae,
		Coral_eat_seaweed,
		Coral_ocean_plastics,
		Coral_hunting_lions,
		Coral_casting_shade,
		Coral_turtle_population,
		Coral_tang_checkup,
		Bayou_dirty_detritus,
		Bayou_reef_decision,
		Turtle_danger,
		Displaced_reef,
		Turtle_danger2,
		Bayou_blue_waters,
		Bayou_hide_n_seek,
		Bayou_oxygen_tracking,
		Bayou_shrimp_yields,
		Bayou_methanogen,
		Bayou_shrimp_tastrophe,
		Bayou_alt_energy,
		Bayou_boom_cause,
		Arctic_disappearing_act,
		Arctic_underneath,
		Arctic_endangered_seals,
		Above_n_below,
		Arctic_time_of_death,
		Arctic_isolated_instance,
		Arctic_picky_eaters,
		Arctic_salmon_monitoring,
		Arctic_missing_whale,
		Arctic_in_ice,
		Arctic_cause_of_death,
		Arctic_salmon_competition,
		Arctic_stationary_survival,
		Arctic_seal_habitats,
		Final_final,
	};

	static public int IndexOf(StringHash32 inJobId)
	{
		return Array.IndexOf(All, inJobId);
	}
}